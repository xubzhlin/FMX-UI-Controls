...
const
  TWeeks: array [0 .. 6] of string = ('周日', '周一', '周二', '周三', '周四',
    '周五', '周六');
    (*
...
function TCalendarControl.CheckDateChanged(NewValue, OldValue: TDate; IsStartDate:Boolean = True): Boolean;
begin
  if ((Yearof(NewValue)<>Yearof(OldValue)) or (MonthOf(NewValue)<MonthOf(OldValue))) and IsStartDate then
    Result:=True
  else
  if ((Yearof(NewValue)<>Yearof(OldValue)) or (MonthOf(NewValue)>MonthOf(OldValue))) and (not IsStartDate) then
    Result:=True
  else
    Result:=False;
end;
...
begin
  ...
  try
    ...
    while (Yearof(FDate)<=Yearof(FEndDate)) and ((FDate<FEndDate) or (Monthof(FDate)<=Monthof(FEndDate))) do
    begin
      FillMonth(FDate);
