...
type
  ...
  private
    ...
    //Fix By Xubzhlin
    FDeleteLayoutWidth: Single;
    FDeleteButtonWidth: Single;
    ...
    //Fix Xubzhlin
    function GetDeleteLayout: TLayout;
    procedure SetIsDeleteCutoff(const Value: Boolean);
  protected
  ...
  public
    ...
    // Fix Bu Xubzhlin
    property DeleteButton:TSpeedButton read FDeleteButton;
    property DeleteLayout:TLayout read GetDeleteLayout;
    property DeleteLayoutWidth:Single read FDeleteLayoutWidth write FDeleteLayoutWidth;
    property DeleteButtonWidth:Single read FDeleteLayoutWidth write FDeleteLayoutWidth;
    property DeleteButtonIndex:Integer read FDeleteButtonIndex write SetDeleteButtonIndex;
    property IsDeleteCutoff:Boolean read FIsDeleteCutoff write SetIsDeleteCutoff;
  end;
  ...
  public
    ...
    property DeleteLayout;
    property DeleteLayoutWidth;
    property DeleteButtonWidth;
  published
    // Hoist protected appearance properties
...
begin
  ...
  //Fix By Xubzhlin
  FDeleteButtonWidth := DefaultDeleteButtonWidth;
  FDeleteLayoutWidth := DefaultDeleteButtonWidth;
  FIsDeleteCutoff := True;
  FMaxKnownHeight := 0;
  FScrollScale := 1;
...
begin
  //fix by xubzhlin 2018.3.28
  //list textitem 固定 不偏移
//  //-----------------------
...
function TListViewBase.GetDeleteLayout: TLayout;
begin
  ...
  begin
    ...
    FDeleteLayout.Width := FDeleteLayoutWidth;
    FDeleteLayout.ClipChildren := True;
    FDeleteLayout.Parent := Self;
  end;
  Result:=FDeleteLayout;
end;
...
begin
  ...
  begin
    ...
    FDeleteButton.Width := FDeleteButtonWidth;
...
begin
  ...
  begin
    FDeleteLayout.Width := FDeleteLayoutWidth * FDeleteModeTransitionAlpha;
...
begin
//fix by xubzhlin
//只要点击 删除按钮 不管是否真正删除，都隐藏DeleteLayout
//<-----------------
...
//<+++++++++++++++++++++
  ...
  if FDeleteLayout.Width = DefaultDeleteButtonWidth then
  begin
    if DeleteItem(FDeleteButtonIndex) then
    begin
      FSelectionAlphas.Remove(FDeleteButtonIndex);
...
begin
  ...
  begin
    ...
    //当使用自定义Header的时候
    //当Header ObjectCount>Item Object Count
    //GetNumberOfRenderingPasses NumberOfPasses 计算不正确
    //Header 不会绘制多余的Object 在此修正
    //<+++++++++++++++++++++++++++++
    ListItem:= Adapter[ItemHeaderIndex];
    NumberOfPasses:=ListItem.Count;
    //++++++++++++++++++++++++++++++>
...
procedure TListViewBase.SetIsDeleteCutoff(const Value: Boolean);
begin
  if FIsDeleteCutoff<>Value then
  begin
    FIsDeleteCutoff := Value;
    Invalidate;
  end;
end;
