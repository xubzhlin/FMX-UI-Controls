...
uses
  ...
  System.Generics.Defaults, Macapi.ObjectiveC, Macapi.CoreFoundation,
  iOSapi.CocoaTypes, iOSapi.CoreGraphics,
  iOSapi.Foundation, iOSapi.CoreText, iOSapi.UIKit, FMX.Types, FMX.Surfaces,
  FMX.FontGlyphs;
...
uses
  System.Math, System.Character, System.Math.Vectors, FMX.Graphics, FMX.Consts,
  FMX.Utils, Macapi.Helpers;
...
const
  ...
  ItalicMatrix: CGAffineTransform = (a: 1; b: 0; c: 0.176326981;
    // ~tan(10 degrees)
    d: 1; tx: 0; ty: 0);
var
...
begin
  Matrix := nil;
  FFontRef := CTFontCreateWithName(GetPostScriptFontName,
    CurrentSettings.Size * CurrentSettings.Scale, nil);
  try
    ...
    begin
      ...
      begin
        ...
        NewFontRef := CTFontCreateWithName(GetPostScriptFontName,
          CurrentSettings.Size * CurrentSettings.Scale, @ItalicMatrix);
        if NewFontRef <> nil then
        begin
          CFRelease(FFontRef);
          FFontRef := NewFontRef;
...
begin
  ...
  CTRunGetTypographicBounds(Run, CFRangeMake(0, 1), @Ascent, @Descent,
    @Leading);
...
begin
  Result := nil;
  LUIFont := TUIFont.Wrap(TUIFont.OCClass.fontWithName
    (StrToNSStr(CurrentSettings.Family), CurrentSettings.Size *
...
procedure PathApplierFunction(info: Pointer;
  const element: PCGPathElement); cdecl;
var
  P, P1, P2: PCGPoint;
begin
  P := element^.points;
  case element.type_ of
    ...
    kCGPathElementAddQuadCurveToPoint:
      begin
        P1 := P;
        Inc(P1);
        TPathData(info).QuadCurveTo(TPointF.Create(P.x, P.y),
          TPointF.Create(P1.x, P1.y));
      end;
    kCGPathElementAddCurveToPoint:
      begin
        ...
        P2 := P1;
        Inc(P2);
        TPathData(info).CurveTo(TPointF.Create(P.x, P.y),
          TPointF.Create(P1.x, P1.y), TPointF.Create(P2.x, P2.y));
      end;
...
function TIOSFontGlyphManager.DoGetGlyph(const Char: UCS4Char;
...
var
  ...
  ContextSize: TSize;
begin
  LImageChar := (Char = $1F004) or (Char = $1F0CF) or (Char = $1F170) or
    (Char = $1F171) or (Char = $1F17E) or (Char = $1F17F) or (Char = $1F18F) or
    ((Char >= $1F191) and (Char <= $1F19A)) or (Char = $1F201) or
    (Char = $1F202) or (Char = $1F21A) or (Char = $1F22F) or
    ((Char >= $1F232) and (Char <= $1F23A)) or (Char = $1F250) or
    (Char = $1F251) or ((Char >= $1F300) and (Char <= $1F320)) or
    ((Char >= $1F330) and (Char <= $1F393)) or
    ((Char >= $1F3A0) and (Char <= $1F3F0)) or
    ((Char >= $1F400) and (Char <= $1F43E)) or (Char = $1F440) or
    ...
    ((Char >= $1F680) and (Char <= $1F6C5)) or (Char = $2139) or
    ((Char >= $2194) and (Char <= $2199)) or (Char = $21A9) or (Char = $21AA) or
    (Char = $231A) or (Char = $231B) or ((Char >= $23E9) and (Char <= $23EC)) or
    (Char = $23F0) or (Char = $23F3) or (Char = $24C2) or (Char = $25AA) or
    (Char = $25AB) or (Char = $25B6) or (Char = $25C0) or
    ((Char >= $25FB) and (Char <= $25FE)) or (Char = $2600) or (Char = $2601) or
    (Char = $260E) or (Char = $2611) or (Char = $2614) or (Char = $2615) or
    (Char = $261D) or (Char = $263A) or ((Char >= $2648) and (Char <= $2653)) or
    (Char = $2660) or (Char = $2663) or (Char = $2665) or (Char = $2666) or
    (Char = $2668) or (Char = $267B) or (Char = $267F) or (Char = $2693) or
    (Char = $26A0) or (Char = $26A1) or (Char = $26AA) or (Char = $26AB) or
    (Char = $26BD) or (Char = $26BE) or (Char = $26C4) or (Char = $26C5) or
    (Char = $26CE) or (Char = $26D4) or (Char = $26EA) or (Char = $26F2) or
    (Char = $26F3) or (Char = $26F5) or (Char = $26FA) or (Char = $26FD) or
    (Char = $2702) or (Char = $2705) or ((Char >= $2708) and (Char <= $270F)) or
    (Char = $2712) or (Char = $2714) or (Char = $2716) or (Char = $2728) or
    (Char = $2733) or (Char = $2734) or (Char = $2744) or (Char = $2747) or
    (Char = $274C) or (Char = $274E) or ((Char >= $2753) and (Char <= $2755)) or
    (Char = $2757) or (Char = $2764) or ((Char >= $2795) and (Char = $2797)) or
    (Char = $27B0) or (Char = $27BF) or (Char = $2934) or (Char = $2935) or
    ((Char >= $2B05) and (Char = $2B07)) or (Char = $2B1B) or (Char = $2B1C) or
    (Char = $2B50) or (Char = $2B55) or (Char = $3030) or (Char = $303D) or
    (Char = $3297) or (Char = $3299);
  ...
  Str := CFStringCreateWithCharacters(kCFAllocatorDefault, PChar(CharsString),
    CharsStringLength);
  ...
  // Metrics
  Context := CGBitmapContextCreate(nil, 1, 1, 8, 4, FColorSpace,
  ...
  try
    ...
    GlyphStyle := [];
    if ((Bounds.Size.width = 0) and (Bounds.Size.height = 0)) or
      not HasGlyph(Char) then
  ...
  if (TFontGlyphSetting.Bitmap in Settings) and
    (HasGlyph(Char) or ((Bounds.Size.width > 0) and (Bounds.Size.height > 0)))
  then
  begin
    ContextSize := TSize.Create(Max(GlyphRect.Right, GlyphRect.width),
      GlyphRect.Bottom);

    Context := CGBitmapContextCreate(nil, ContextSize.width, ContextSize.height,
      8, ContextSize.width * 4, FColorSpace, kCGImageAlphaPremultipliedLast);
    try
      ...
      if not SameValue(FDefaultBaseline - BaseLineOffset, 0, TEpsilon.position)
      then
        CGContextTranslateCTM(Context, 0,
          -Abs(FDefaultBaseline - BaseLineOffset));
      CTFrameDraw(Frame, Context);

      Result.Bitmap.SetSize(GlyphRect.width, GlyphRect.height,
        TPixelFormat.BGRA);
        ...
        for I := GlyphRect.Left to GlyphRect.Right - 1 do
          ...
          begin
            ...
            begin
              ...
              Result.Bitmap.Pixels[I - GlyphRect.Left, J - GlyphRect.Top] :=
