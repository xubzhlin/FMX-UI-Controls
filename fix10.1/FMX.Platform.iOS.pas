...
{$M+}
  ...
  private
    ...
    procedure SetStatusColor(const ABackgroundColor, ATitileColor: TAlphaColor);
...
procedure TFMXViewController.SetStatusColor(const ABackgroundColor,
  ATitileColor: TAlphaColor);
var
  Color: UIColor;
begin
  Color := AlphaColorToUIColor(ABackgroundColor);
  FStatusBar.setBackgroundColor(Color);
  FStatusBarLuminance := Luminance(ATitileColor);
  ViewController.setNeedsStatusBarAppearanceUpdate;
...
begin
  Result := (AForm <> nil) and (AForm.BorderStyle <> TFmxFormBorderStyle.None){+++>} and (AForm.BorderStyle <> TFmxFormBorderStyle.ToolWindow){<+++}; // by ??
end;
...
{+++>}RootViewController.StatusBarVisible := HasFormStatusBar(AForm) or (AForm.BorderStyle = TFmxFormBorderStyle.ToolWindow); // by ??
    if AForm is TCustomForm then
{+++>}
      if AForm.BorderStyle = TFmxFormBorderStyle.ToolWindow then
           //RootViewController.StatusBarColor := TAlphaColorRec.Null
           //fix by xubzhlin
           RootViewController.SetStatusColor(TAlphaColorRec.Null, TCustomForm(AForm).Tag and $FFFFFFFF)
      else
{<+++}
